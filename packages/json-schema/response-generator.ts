...
export class ResponseGenerator {
  ...
  private generateFromSchema(
    ...
    if (
      ...
      this.options.useExamples !== false
    ) {
  ...
  private generateNumber(
    ...
    // JSON Schema 2020-12: exclusiveMinimum and exclusiveMaximum are numbers, not booleans
    let min = 0;
    let max = 100;

    // Handle minimum/exclusiveMinimum (exclusiveMinimum takes precedence)
    if (schema.exclusiveMinimum !== undefined) {
      // exclusiveMinimum: value must be GREATER than this
      min = schema.exclusiveMinimum + (integer ? 1 : Number.EPSILON);
    } else if (schema.minimum !== undefined) {
      // minimum: value must be GREATER THAN OR EQUAL to this
      min = schema.minimum;
    }

    // Handle maximum/exclusiveMaximum (exclusiveMaximum takes precedence)
    if (schema.exclusiveMaximum !== undefined) {
      // exclusiveMaximum: value must be LESS than this
      max = schema.exclusiveMaximum - (integer ? 1 : Number.EPSILON);
    } else if (schema.maximum !== undefined) {
      // maximum: value must be LESS THAN OR EQUAL to this
      max = schema.maximum;
    }
  ...
  private generateString(schema: Schema, context: GenerateContext): string {
    ...
    const length = minLength +
      Math.floor(context.random.next() * (maxLength - minLength + 1));
  ...
  private generateFormat(
    ...
    switch (format) {
      case "date-time":
        return new Date(
          Date.now() -
            Math.floor(context.random.next() * 365 * 24 * 60 * 60 * 1000),
        )
          .toISOString();
      ...
      case "ipv6":
        return Array(8).fill(0).map(() =>
          Math.floor(context.random.next() * 65536).toString(16).padStart(
            4,
            "0",
          )
        ).join(":");
      ...
      case "uri":
        return `https://example.com/path${
          Math.floor(context.random.next() * 1000)
        }`;
  ...
  private generateArray(
    ...
    const length = minItems +
      Math.floor(context.random.next() * (maxItems - minItems + 1));
    ...
    if (schema.uniqueItems && array.length > 1) {
      ...
      const safeStringify = (value: unknown): string => {
        try {
          ...
          // For circular refs or other issues, use type-based key
          return `<${typeof value}:${Date.now()}:${Math.random()}>`;
        }
      };
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        seen.add(safeStringify(array[i]));
      }
    }
