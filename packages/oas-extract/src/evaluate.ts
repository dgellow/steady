import type { OpenAPISpec, ExtractedSchema } from "./types.ts";
...
export async function evaluateStability(
  ...
  for (let i = 0; i < runs; i++) {
    ...
    if (verbose) {
      console.log(`  Extracted ${result.extracted.length} schemas in ${(elapsed / 1000).toFixed(1)}s`);
    }
  }
  
  // Calculate metrics
  const consistency = calculateConsistency(results);
  const uniqueNamesPerRun = results.map(r => new Set(r.map(s => s.name)).size);
...
function calculateConsistency(results: ExtractedSchema[][]): number {
  ...
  for (const runResults of results) {
    for (const schema of runResults) {
      const key = `${schema.context.method || ""}::${schema.context.path}::${schema.context.location}`;
...
function findVariations(results: ExtractedSchema[][]): NameVariation[] {
  ...
  const namesByLocation = new Map<string, { path: string; method: string; names: string[] }>();
...
function generateComparisonSummary(reports: StabilityReport[]): string {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    summary += `${i + 1}. **${report.strategy}**: ${(report.consistency * 100).toFixed(1)}% consistent\n`;
  }
  
  summary += "\n### Performance\n";
  for (const report of reports) {
    summary += `- **${report.strategy}**: ${(report.avgExtractionTime / 1000).toFixed(1)}s average\n`;
  }
  ...
  for (const report of reports) {
    ...
    if (totalVariations === 0) {
      ...
      const avgConsistency = report.variations.reduce((sum, v) => sum + v.consistencyRate, 0) / totalVariations;
      summary += `- **${report.strategy}**: ${totalVariations} variations (avg ${(avgConsistency * 100).toFixed(1)}% consistent)\n`;
    }
  }
...
export function formatStabilityReport(report: StabilityReport): string {
  ...
  output += `**Overall Consistency**: ${(report.consistency * 100).toFixed(1)}%\n`;
  output += `**Average Extraction Time**: ${(report.avgExtractionTime / 1000).toFixed(1)}s\n`;
  output += `**Unique Names per Run**: ${report.uniqueNamesPerRun.join(", ")}\n`;
  ...
  if (report.variations.length > 0) {
    ...
    for (const variation of report.variations.slice(0, 10)) {
      output += `**${variation.method} ${variation.path}** (${(variation.consistencyRate * 100).toFixed(0)}% consistent)\n`;
