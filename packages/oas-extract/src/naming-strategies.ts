...
function detectCommonPattern(schema: SchemaObject): boolean {
  ...
  if (
    props.includes("error") &&
    (props.includes("message") || props.includes("code"))
  ) {
    return true;
  }
  ...
  if (
    props.includes("page") || props.includes("limit") ||
    props.includes("offset") ||
    props.includes("total") || props.includes("next") ||
    props.includes("previous")
  ) {
    return true;
  }
  ...
  if (
    props.length === 1 &&
    (props[0] === "data" || props[0] === "items" || props[0] === "results")
  ) {
    return true;
  }
  ...
  // Metadata pattern
  if (
    props.includes("meta") || props.includes("metadata") ||
