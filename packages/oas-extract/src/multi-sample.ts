...
import type { NamingContext, NamingResult, NamingStrategy } from "./naming-strategies.ts";
...
export async function generateWithMultiSample(
  ...
  const samples = await Promise.all(
    Array(strategy.samples).fill(0).map(() => 
      generateSingleBatch(groups, existingSchemaNames, llmClient, temperature, buildPrompt)
    )
  );
...
function selectMostCommon(decisions: DeduplicationDecision[]): DeduplicationDecision {
  // Count occurrences of each name
  const nameCounts = new Map<string, { count: number; decision: DeduplicationDecision }>();
...
function selectBestScore(decisions: DeduplicationDecision[]): DeduplicationDecision {
...
function scoreNameQuality(name: string): number {
  ...
  const meaningfulWords = ["User", "Error", "Page", "Response", "Request", "Config", 
                           "Meta", "Filter", "Sort", "Query", "Result", "Status"];
